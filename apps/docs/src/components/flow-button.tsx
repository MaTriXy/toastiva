...
const Slot = React.forwardRef<HTMLElement, SlottableProps>(
  ({ children, className, ...props }, ref) => {
    if (!React.isValidElement<SlottableProps>(children)) {
      return null;
    }
    ...
    return React.cloneElement(children, {
      ...props,
      ...children.props,
      ...(ref ? ({ ref } as React.Attributes) : null),
      className: cn(className, children.props.className),
    } as Partial<SlottableProps> & React.Attributes);
  },
);
...
const FlowButton = React.forwardRef<HTMLButtonElement, FlowButtonProps>(
  (
    ...
    const buttonRef = React.useRef<HTMLElement | HTMLButtonElement>(null);
    ...
    return (
      <>
        ...
        <div className="relative inline-block group pointer-events-none">
          ...
          <Comp
            ref={buttonRef as React.Ref<HTMLButtonElement> & React.Ref<HTMLElement>}
            className={cn(
              "relative z-0 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 pointer-events-auto cursor-pointer inline-flex items-center justify-center gap-2 whitespace-nowrap font-[550] transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-100 dark:bg-muted/50 text-primary dark:hover:bg-transparent hover:bg-transparent",
              buttonSize,
              className,
            )}
            {...props}
